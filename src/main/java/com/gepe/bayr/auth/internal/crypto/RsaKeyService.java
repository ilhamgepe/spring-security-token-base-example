...
public class RsaKeyService {
    ...
    public RSAKey loadActiveRsaKey(){
        SigningKey activeKey = signingKeyRepo.findTopByIsActiveTrueOrderByCreatedAtDesc()
                .orElseThrow(() -> new IllegalStateException("No active signing key – this should never happen"));
    ...
    @Transactional(readOnly = true)
    public List<RSAKey> loadPublicKeysForJwks(){
        return signingKeyRepo.findAllNotExpiredOrActive(Instant.now()).stream()
                .map(k -> parsePublicRsaKeyOnly(k.getKid(), k.getPublicKey()))
                .toList();
    }
